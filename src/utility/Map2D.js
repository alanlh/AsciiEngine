class Map2D {
  ...
  insertSet2D(id, set2d) {
    for (let vec2 of set2d) {
      if (this.contains(vec2)) {
        // TODO: Do we want this behavior for arbitrary Map2D? This seems specific to CollisionDetector...
        // Make sure the current spots aren't occupied already. 
        return false;
      }
    }
    for (let vec2 of set2d) {
